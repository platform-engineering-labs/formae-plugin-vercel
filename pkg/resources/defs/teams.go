...
//
// Discovery is off (`discoverable = false` in schema/pkl/core/teams.pkl), and
// the reason changed when the issue flow was removed.
//
// GET /v8/certs returns issued and uploaded certificates in one
// undifferentiated list, with no field telling them apart. Discovery would
// therefore adopt certificates Vercel issued itself — one appears automatically
// whenever a domain is attached to a project — and those cannot be deleted:
//
//	DELETE /v8/certs/{id}
//	400 SSL Certificates provided by the system cannot be deleted.
//
// Adopting them would hand formae resources it can never destroy, which is
// exactly why VERCEL::Certs::Certificate is not declared at all. So this
// resource manages what a forma uploads and discovers nothing.
