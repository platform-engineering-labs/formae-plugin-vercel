...
// The issue flow is not declared here, and the guard now runs the other way:
// nothing may quietly turn the upload resource into the issue endpoint.
//
// VERCEL::Certs::Certificate was removed because Vercel will not delete a
// certificate it issued — DELETE /v8/certs/{id} answers "SSL Certificates
// provided by the system cannot be deleted" — so formae could create one and
// never destroy it, and any stack containing one could never be torn down.
func TestOnlyTheUploadFlowIsDeclared(t *testing.T) {
	var uploaded *rest.Definition
	for _, def := range All() {
		if def.Type == "VERCEL::Certs::Certificate" {
			t.Error("the issue flow is undeletable and must not be declared")
		}
		if def.Type == "VERCEL::Certs::UploadedCertificate" {
			c := def
			uploaded = &c
	...
	if uploaded.CreateMethod != http.MethodPut {
		t.Errorf("the upload flow must stay a PUT, got %q", uploaded.CreateMethod)
	}
	if contains(uploaded.Fields, "cns") {
		t.Error("cns is derived from the uploaded certificate, not an input")
	}
	for _, secret := range []string{"cert", "ca", "key"} {
		if !contains(uploaded.Fields, secret) {
			t.Errorf("the upload flow needs %q", secret)
		}
	}
}
